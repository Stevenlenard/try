<?php
// ===== OPTION B (AGGRESSIVE — DESTROY + NEW SESSION) =====
// Destroys any active session and starts a fresh one. Use only if you want to
// remove ALL session data (complete logout / fresh state).
...
// Start a brand new session safely
if (session_status() !== PHP_SESSION_ACTIVE) {
    session_start();
}
...
// Determine current user id in a safe way so we don't reference undefined variables.
// Many parts of this file already use getCurrentUserId() later; define $currentUserId early.
$currentUserId = null;
if (function_exists('getCurrentUserId')) {
    ...
    // fallback if other parts of the app define $userId earlier
    $currentUserId = (int)$userId;
}

// NOTE: Removed the unconditional broadcast that used an undefined $userId.
// If you want to broadcast an event when creating or updating notifications,
// we now broadcast right after DB inserts/updates below (see the mark_read / insert sections).
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'mark_all_read') {
            ...
            // Broadcast an update so connected clients can refresh/update UI without reload
            if (function_exists('send_ws_broadcast')) {
                send_ws_broadcast([
                    'type' => 'notifications_bulk_update',
                    'payload' => ['action' => 'mark_all_read', 'by_user' => $currentUserId]
                ]);
            }
        ...
        if ($action === 'mark_read') {
            ...
            if (!empty($_POST['notification_id'])) {
                ...
                // Broadcast that a single notification was marked read
                if (function_exists('send_ws_broadcast')) {
                    send_ws_broadcast([
                        'type' => 'notification_read',
                        'payload' => ['notification_id' => $id, 'by_user' => $currentUserId]
                    ]);
                }
            ...
            $currentUserId = $currentUserId ?: (function_exists('getCurrentUserId') ? getCurrentUserId() : null);
            ...
            if (isset($pdo) && $pdo instanceof PDO) {
                ...
                // Broadcast the newly created notification so clients see it instantly
                if (function_exists('send_ws_broadcast')) {
                    send_ws_broadcast([
                        'type' => 'notification',
                        'payload' => [
